...
from datasets.scannet200.owis_splits import UNKNOWN_CLASSES_LABELS
############################################################################################################################
# matrix: 4x4 np array
# points Nx3 np array
...
def get_instances(split,task, ids, class_ids, class_labels, id2label, pretrained_dataset='scannet200'):
    instances = {}
    ...
    for id in instance_ids:
        ...
        # TAIL_LABELS = TAIL_CATS_SCANNET_200
        if pretrained_dataset == 'scannet200':
            unknown_class_labels = UNKNOWN_CLASSES_LABELS[split][task]
            #get their ids
            try:
                # if task == "task1":
                ...
                #         inst.label_id = 3000
                # elif task == "task2":
                #     if id2label[inst.label_id] in TAIL_LABELS:
                #         inst.label_id = 3000
                if unknown_class_labels is not None:
                    if id2label[inst.label_id] in unknown_class_labels:
                        inst.label_id = 3000
            except:
                pass
            # print(inst.label_id)
