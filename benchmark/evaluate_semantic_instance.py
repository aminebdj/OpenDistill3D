                        ...
                        for pred in gt['matched_pred']:
                            ...
                            if overlap > overlap_th:
                                ...
                                if cur_match[gti]:
                                    ...
                                    # unknown_id = 3000 #scannet200
                                    unknown_id = 3000 #stpl3d
                                    
                                    if pred['pred_id'] != unknown_id and gt['label_id'] == unknown_id:
                                        A_OSE[round(overlap_th,2)] += 1
                                    if pred['pred_id'] != unknown_id and gt['label_id'] == unknown_id and pred['confidence']>0.5:
                                        FPo[round(overlap_th,2)] += 1
                                    if pred['pred_id'] != unknown_id and gt['label_id'] != unknown_id and gt['label_id'] != pred['pred_id'] and pred['confidence']>0.5:
                                        FPc[round(overlap_th,2)] += 1
                                    if pred['pred_id'] == gt['label_id'] and gt['label_id'] != unknown_id and pred['confidence']>0.5:
...
def assign_instances_for_scan(split,task, pretrained_dataset, pred: dict, gt_file: str):
    ...
    # get gt instances
    gt_instances = util_3d.get_instances(split,task, gt_ids, VALID_CLASS_IDS, CLASS_LABELS, ID_TO_LABEL, pretrained_dataset)
    # associate
...
# 488, 540, 562, 570, 572, 581, 609, 748, 776, 1156, 1163, 1164, 1165, 1166, 1167, 1168, 1169, 1170, 1171, 1172, 1173, 1174, 1175, 1176, 1178, 1179, 1180, 1181, 1182, 1183, 1184, 1185, 1186, 1187, 1188, 1189, 1190, 1191,3000))
    ...
    for i,(k,v) in enumerate(preds.items()):
        ...
        gt2pred, pred2gt = assign_instances_for_scan(config_general.split,config_general.OW_task, config_general.pretrained_dataset, v, gt_file)
        matches[matches_key] = {}
