...
class Mask3D(nn.Module):
    ...
    def forward(self, x, point2segment=None, raw_coordinates=None, is_eval=False):
        ...
        backbone_mapped_feats = mask_features
        ...
        return {
            ...
            'queries_list': torch.stack(mul_queries).permute(1,2,0,3),
            "semantic_features": semantic_features,
            "backbone_mapped_feats": backbone_mapped_feats
        }
    ...
    def _set_aux_loss(self, outputs_class, outputs_seg_masks):
        ...
        return [
            # {"pred_logits": a, "pred_masks": b, "obj":(torch.max(self.softmax(a),dim=-1).values)*torch.stack([torch.sum(_.float().sigmoid(),dim=0)/(torch.sum((_.float().sigmoid()>0.5).float(),dim=0)+0.0001) for _ in b])}
