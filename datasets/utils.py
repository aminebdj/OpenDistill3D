...
##########################
from datasets.scannet200.scannet200_constants import MAP_STRING_TO_ID
...
def get_instance_masks(list_labels, task, list_segments=None, ignore_class_threshold=100, 
    ...
    for batch_id in range(len(list_labels)):
        # #####################################################
        # COMMON_TAIL_CATS_SCANNET_200_IDS = list(map(lambda k:MAP_STRING_TO_ID[k], COMMON_CATS_SCANNET_200+TAIL_CATS_SCANNET_200))
    
        # for k in COMMON_TAIL_CATS_SCANNET_200_IDS:
        #     try:
        #         list_labels[batch_id][:, 0][list_labels[batch_id][:, 0]==k]=255
        #         list_segments[batch_id][:, 0][list_segments[batch_id][:, 0]==k]=255
        #     except:
        #         print('exception occured')
        # #####################################################
        # # check if any TAIL COMMON classes are there l = sum(list(map(lambda k: torch.mean((list_labels[batch_id][:, 0]==k).float()).item(),COMMON_TAIL_CATS_SCANNET_200_IDS)))
        label_ids = []
        masks = []
        segment_masks = []
